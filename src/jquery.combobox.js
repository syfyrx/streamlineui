 ...
 (function($) {
	 ...
	 */
	 function create(target) {
	 	...
	 	$(target).bind('focus.combobox',function(e){
	 		state.panel.css({
	 	...
	 	state.panel.delegate('div.combobox-item', 'mouseenter.combobox', function(e) {
	 	...
	 	for(var e in opts.inputEvents){
			$(target).bind(e+".combobox",{target:target},opts.inputEvents[e]);
		}
	 }
	/**
	 * 显示面板
	 ...
	 * 设置组件值
	 */
	 function setValues(target,newValues) {
	 	var state = $(target).data('combobox');
	 	var opts = state.options;
	 	var panel = state.panel;
	 	...
	 	opts.onChange.call(vv,state.values);
	 	state.values=vv;
	...
	function readonly(target,param){
		var state = $(target).data('combobox');
	 	...
	 	opts.readonly=param;
	 	if(param){
	 		$(target).addClass('combobox-readonly').attr('readonly',true);
