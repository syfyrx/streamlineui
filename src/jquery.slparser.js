...
(function($) {
	$.slparser = {
		...
		plugins : [ 'sldraggable', 'sldroppable', 'slresizable', 'slpagination', 'sltooltip', 'sllinkbutton', 'slmenu', 'slmenubutton', 'slsplitbutton', 'slprogressbar', 'sltree', 'sltextbox', 'slfilebox', 'slcombo', 'slcombobox', 'slcombotree', 'slcombogrid', 'slnumberbox', 'slvalidate', 'slsearchbox', 'slspinner', 'slnumberspinner', 'sltimespinner', 'sldatetimespinner', 'slcalendar', 'sldatebox', 'sldatetimebox', 'slslider', 'sllayout', 'slpanel', 'sldatagrid', 'slpropertygrid', 'sltreegrid', 'sltabs', 'slaccordion', 'slwindow', 'sldialog', 'slform' ],
		parse : function(context) {
			var aa = [];
			for (var i = 0; i < $.slparser.plugins.length; i++) {
				var name = $.slparser.plugins[i];
				var r = $('.slui-' + name.substring(2, name.length), context);
				if (r.length) {
					if (r[name]) {
						r[name]();
					} else {
			...
			if (aa.length && window.loader) {
				...
				loader.load(names, function() {
					...
					$.slparser.onComplete.call($.slparser, context);
				});
			} else {
				$.slparser.onComplete.call($.slparser, context);
			}
		 ...
		 * 
		 * calling examples: $.slparser.parseOptions(target);
	...
	$(function() {
		if (!window.loader && $.slparser.auto) {
			$.slparser.parse();
		}
	});
	...
	$.fn._size = function(options, parent) {
		...
		function _setSize(target, property, parent, options) {
			...
			var min = $.slparser.parseValue('min' + p1, options['min' + p1], parent);// ||
																						// 0;
			var max = $.slparser.parseValue('max' + p1, options['max' + p1], parent);// ||
																						// 99999;
			var val = $.slparser.parseValue(p, options[p], parent);
